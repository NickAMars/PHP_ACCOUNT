...
if (!file_exists("assignment11-account-info.txt") ){ //if it does exitst

	$myfile = fopen("assignment11-account-info.txt", "w") or die("Unable to open file!");
fclose($myfile);
}
...
if (isset($_GET["signin"])) {
    ...
    $file1 = fopen("assignment11-account-info.txt", "r");
    while (!feof($file1)) {
        $line = fgets($file1);
        if (strlen($line) > 0) {
            $arr = explode(";", $line);
...
if ($_GET["uname2"]=="" ||$_GET["psw2"]=="" ||$_GET["fname"]==""||$_GET["lname"]=="" ){
    ...
    $_SESSION['url'] = "http://r.ddmcdn.com/s_f/o_1/cx_633/cy_0/cw_1725/ch_1725/w_720/APL/uploads/2014/11/too-cute-doggone-it-video-playlist.jpg";
    $file = fopen("assignment11-account-info.txt", "a");
...
$fileContent.= $_SESSION['uname'] . ";" . $arr[1] . ";" . $_SESSION['fname'] . ";" . $_SESSION['lname'] . ";" . $_SESSION['color'] . ";" . $_SESSION['title'] . ";" . $_SESSION['url'] . "\n";
        ...
        // Write the content
        $file2 = fopen("assignment11-account-info.txt", "w") or exit("Unable to open file!");
